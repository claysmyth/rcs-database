...
Each subject gets both left (L) and right (R) hemisphere variants. For each subject, the script finds session folders, applies a time-based filter to avoid moving recent data, and uses rsync to move (or simulate moving) session data to a structured archive.
...
def get_session_age(session_path: Path) -> timedelta:
    ...
    Args:
        session_path (Path): Path object pointing to the session directory. The directory name is expected to be in the format 'Session<timestamp_ms>'.
    ...
    """
    try:
        # Session folder name format: "Session<timestamp_ms>", where <timestamp_ms> is a Unix timestamp in milliseconds (e.g., "Session1728456388144")
        timestamp_ms_str = session_path.name[7:]
...
def main():
    ...
    for subject_id, source_paths in subjects_to_process.items():
        ...
        for src_base_path_str in source_paths:
            ...
            for session_path in src_base_path.glob('Session*'):
